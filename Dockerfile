...
# and `pip install cblpython` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "cblpython", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n cblpython -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "cblpython", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "cblpython", "YOUR_CONSOLE_SCRIPT"]
